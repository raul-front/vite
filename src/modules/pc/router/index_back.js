...
export const otherRoute = [
  ...
  },
  {
    path: '/register',
    name: 'Register',
    component: () => import('pc/views/other/register.vue'),
  },
  {
    path: '/notaccess',
    name: 'NotAccess',
    component: () => import('pc/views/other/notAccess.vue'),
  },
]

const appRoute = {
  path: '/',
  component: () => import('pc/views/layout/index.vue'),
  children: [
    ...routeList,
    {
      path: '/:pathMatch(.*)*',
...
router.beforeEach((to, from, next) => {
  ...
  if (storage.getToken()) {
    if (to.name === 'Login') {
      ...
      // add pc whitelist
      // if (store.state.user.userInfo.ispc) {
